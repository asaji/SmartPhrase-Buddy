...
Templates own independent content; no shared content sections (account-level shared *choice variables* — option lists referenced as `[[@Label]]` — are separate; see "Shared choice variables"). Each save creates a full immutable revision snapshot, with optimistic version checks. Restore creates another revision. AI proposal requests do not write templates. Master approval requires a signed, user-bound, expiring selected-ID/version scope. Case proposals cannot be approved through the master endpoint. Browser case drafts never use localStorage, sessionStorage, or analytics; they persist only when the surgeon explicitly clicks **Save to my account**, which keeps one owner-scoped, auto-expiring working copy per operative/procedure master (`CaseDraft`, see "Resumable case drafts").
...
- **Resumable case drafts.** In the operative/procedure case editor, **Save to my account** persists the current working narrative (with an optional short note) as one owner-scoped `CaseDraft` row per master template — so a case begun on one computer can be finished on another. An explicit save overwrites the previous one (one draft per template); nothing is autosaved. Only the narrative is stored — never AI output, free-text instructions, the modifier-22 draft, or the session draft-history checkpoints. Rows auto-delete 7 days after the last save (pruned on every list/read/save) and are **excluded from the portable JSON export/backup**. The library preview shows **Resume saved case** for a template that has one; the editor flags it when the master has advanced since the draft was started; **Finish and clear** offers to delete the saved copy. `GET/POST /api/drafts/`, `GET/DELETE /api/drafts/<id>/`, all `login_required` and owner-scoped.
- **Shared choice variables.** Settings → *Shared choice variables* holds account-level named option lists (`SharedChoice`: `label` + `options[]`, one per name, case-insensitive). A template references one as `[[@Label]]`; the case/procedure "Pick built-in options" panel then populates that field's choices from the shared list, and the declare-once logic makes one control fill every `[[@Label]]` occurrence in the note. Editing a list once updates every template that references it — the point is not re-typing (or re-editing) an assistant roster in every op note. A local `[[Label: … ]]` declaration still overrides; a `[[@Label]]` with no matching shared variable falls back to a free-text field with a warning. Deterministic client-side fill only — nothing here is sent to the AI. Included in the portable JSON export/import. `GET/POST /api/choices/`, `DELETE /api/choices/<id>/`, and `GET /api/status/` carries the resolution map.
...
A template imported from an older PDF export (or pasted from one) can carry Epic's fixed-column hard word-wrap as one `<p>` per printed line, which then pastes into Epic with a hard break mid-paragraph at every wrap point. **Reflow wrapped lines** (below the master editor's content box) runs the same deterministic reflow the importer uses on the open editor content — merging wrapped prose fragments back into flowing paragraphs while keeping `Label:` lines, bulleted/numbered lines, `[[ … ]]` markers and blank-line section breaks. It's client-side, no AI; you review the result in the editor (editor undo reverts it) and it isn't stored until you save a new master revision.
...
Case prompts, provider outputs, and unsaved drafts are not stored in the application database/session, browser storage, analytics or app error logs. They are transiently present in browser/server/provider memory. The one exception is **Save to my account** (see "Resumable case drafts"): it writes the current working narrative to a `CaseDraft` row on the surgeon's explicit click — owner-scoped, one per master template, auto-deleted after 7 days, never containing AI output or instructions, and excluded from the JSON export. Everything else here still holds: navigation clears the UI; memory garbage collection, OS swap/crash reporting, browser extensions/session recovery, clipboard history, and hosting infrastructure are outside that guarantee. Copying does not clear a draft. Finish and clear does not clear your operating system clipboard. A completed AI request may outlive a closed page; it still has no application persistence path. Because a saved `CaseDraft` is real clinical narrative at rest, the deployment requirements below (approved host, private disk, backups, access control) apply to it even though no MRN/name/DOB is used — removing the Epic header does not de-identify the note.
...
Backend tests exercise search dimensions, authentication/ownership/CSRF, temporary cases leaving master/history unchanged, case header exclusion in UI-shaped requests, selected scope, rejection/no-save, approval/restore/stale checks, unfamiliar tokens and explicit removal review, AI failure and malformed response handling, conservative conflict/time checks, the line-by-line finalize endpoint (question build, answer integration, nothing persisted, validation/auth), export/import metadata equivalence/atomic rollback, safe rendering and rate limits, and PDF SmartPhrase extraction (segment splitting, false-positive ALL-CAPS lines, review flags, conservative HTML, hard-wrap reflow keeping label/numbered lines, non-PDF rejection) and the persistent import queue (dedup on re-upload, list/skip/import/restore/clear, bulk skip-all/restore-all leaving imported rows untouched, owner scoping, banner count), resumable case drafts (`CaseDraft` save/resume/overwrite as one row per template, operative/procedure kinds only, content sanitized, `stale_base` when the master advances, 7-day prune on access, owner scoping/auth, and exclusion from the JSON export), and shared choice variables (`SharedChoice` create/list/case-insensitive upsert/delete, label + option normalization and validation, owner scoping/auth, `status` map, and export/import round-trip).

Both browser scripts create a disposable SQLite database and local server with the mock AI provider; they never open the working database or call an external AI provider. Browser smoke tests create and delete a random synthetic account. They exercise sign-in, import, narrative split, manual revision, plain/HTML clipboard, case customization, accepted proposal, draft clear, selected master approval, responsive width and settings, and assert no browser storage or JavaScript errors. Screenshots are written to `/private/tmp` for local review. The regression suite covers repeated procedure generation, manual-edit protection, operative `[[ … ]]` choice fields coexisting with the AI checklist, declare-once choice variables filling every `[[Label]]` reference from a single control (with undeclared-variable warnings in the fill panel and the master editor), the master editor's **Reflow wrapped lines** merging Epic hard-wrapped `<p>` fragments while keeping label lines, the import queue's **Skip all** / **Restore all** bulk actions, the master editor's AI-improve proposal (staged into the editor, persisted only on save), **Save to my account** then resuming that draft from the library preview and deleting it on Finish and clear, a shared choice variable defined in Settings resolving `[[@Assistant]]` across a template and following a roster edit, stale/reused checklists, overlapping sentence removals and fills, formatted placeholders, and delayed grammar responses in every editor. Synthetic grammar responses exercise the review flow without an external AI call.
...
Portable JSON export includes templates, metadata, original source, every revision, and your shared choice variables, but not accounts and not saved `CaseDraft` rows. Settings → Review a portable backup → Approve import adds template copies under the logged-in account and upserts the shared choice variables by name. It preserves timestamps and revision numbers with new local IDs.
...
Screenshot/OCR extraction (image-only PDFs and screenshots; text-based Epic PDF export is implemented); saved *finished* case histories and more than one saved draft per template (one opt-in resumable working copy per template is implemented — see "Resumable case drafts"); reusable case-variation saving; shared-*section* propagation (account-level shared *choice variables* are implemented — see "Shared choice variables"); Epic integration; MFA/SSO and formal production security review; real-provider integration/evaluation; domain deployment and actual Epic paste verification. No exact clinical contents were fabricated from missing screenshots.
