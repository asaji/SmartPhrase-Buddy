...
**Import queue.** `POST /api/import/pdf/` stores every segment as a `PendingImport` row (owner-scoped), deduplicated by `(name, text)` against everything you have ever queued, so uploading the same PDF twice adds nothing. The queue is listed by `GET /api/imports/` (`?dismissed=1` for the skipped list) and rendered right in the import panel — no re-upload needed on later visits — and the library shows a banner with the count. Per entry: **Load this phrase for review** drops it into the import source editor and prefills the title/Epic name; you then set type/tags, click **Use pasted source**, adjust the Epic header boundary, and **Approve**. On save the entry is marked imported (`POST /api/imports/<id>/resolve/ {action:"imported"}`, linked to the new template) and leaves the queue. **Skip** marks it dismissed (`{action:"dismiss"}`, reversible with `restore`). **Clear imported & skipped** (`POST /api/imports/clear/ {scope:"resolved"}`) deletes resolved rows; templates you already saved are untouched.
