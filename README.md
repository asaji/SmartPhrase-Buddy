...
The case editor has a second pass beside the free-text box: **Complete the case, line by line** (`POST /api/finalize/`). **Build case checklist** scans the working narrative for every `***` fill-in (deterministic, client-side — each is shown with its sentence and the `***` marked) and asks the provider for targeted questions about defaults that may not apply to this case (laterality, node-dissection extent, drains, specimens, blood loss, implants, hemostatic agents) and anything commonly documented that looks missing. The prompt never asks for operative time or billing justification.

For each fill-in you choose **Fill in with my text**, **Keep the `***` placeholder**, or **Remove this line** (removals are confirmed and listed first). **Apply answers** inserts your fill-in text verbatim and drops removed lines — a deterministic pass with no model involvement — then, if you answered any checklist questions or ticked *"have the AI phrase my fill-ins"*, sends the result plus your answers back through the same edit contract and shows the outcome in the highlighted-diff review card. Nothing here touches the master template or is persisted; the same token/time/billing safety checks run on the AI pass.
